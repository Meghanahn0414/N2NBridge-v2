...
import { DIRECTORY_BASE } from "../config";

// Plain, unauthenticated client — the Directory Service holds no citizen
// data, just the routing registry, so no bearer token is needed here.
const directoryClient = axios.create({ baseURL: DIRECTORY_BASE, timeout: 15000 });
...
export async function fetchConstituencies(repType: RepType): Promise<ConstituencyOption[]> {
  const { data } = await directoryClient.get("/api/directory/constituencies", {
...
export async function resolveRepresentative(
  ...
  const { data } = await directoryClient.get("/api/directory/resolve", { params });
