...
export default function FieldProfile() {
  ...
  async function loadProfile() {
    try {
      ...
      setProfile({ fullName: user?.name || "Officer", email: user?.email || "" });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const initials = (profile?.fullName || user?.name || "F")
  ...
  return (
    ...
    >
      ...
      <View style={styles.card}>
        ...
        <MenuRow icon="✅" label="My Tasks" onPress={() => router.push("/field/tasks" as any)} />
        <MenuRow icon="🚨" label="Active Alerts" onPress={() => router.push("/field/alerts" as any)} />
      </View>
      ...
      <Text style={styles.version}>Jan Seva CRM · Field Officer App</Text>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: "#D97706", paddingTop: 56, paddingBottom: 32, alignItems: "center" },
  avatarCircle: {
