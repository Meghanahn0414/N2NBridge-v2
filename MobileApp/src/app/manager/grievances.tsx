...
export default function ManagerGrievances() {
  ...
  const FILTERS = ["ALL", "NEW", "ASSIGNED", "IN_PROGRESS", "RESOLVED"];
  ...
  const load = useCallback(async () => {
    try {
      ...
      setItems((data.items ?? data).map((g: any) => ({
        id: g._id || g.id,
        title: g.title || g.subject || "Untitled",
        status: g.status || "NEW",
        category: g.category,
        citizenName: g.citizenName || g.citizen_name,
        assignedOfficerName: g.assignedOfficerName || g.officer_name,
              ...
              <View key={g.id} style={styles.card}>
                <View style={styles.cardTop}>
                  ...
                  <View style={[styles.statusBadge, { backgroundColor: color.bg }]}>
                    <Text style={[styles.statusText, { color: color.text }]}>{g.status.replace("_", " ")}</Text>
                  </View>
                </View>
                <View style={styles.metaRow}>
                  {g.citizenName && <Text style={styles.metaText}>👤 {g.citizenName}</Text>}
                  {g.assignedOfficerName && <Text style={styles.metaText}>🧑‍💼 {g.assignedOfficerName}</Text>}
                </View>
                <View style={styles.cardBottom}>
                  {g.category && <Text style={styles.tag}>{g.category}</Text>}
                  {g.createdAt && <Text style={styles.date}>{new Date(g.createdAt).toLocaleDateString("en-IN")}</Text>}
                </View>
              </View>
            );
          })
...
const styles = StyleSheet.create({
  ...
  search: {
    ...
    fontSize: 14, color: "#111827", elevation: 1, borderWidth: 1, borderColor: "#E5E7EB",
  },
  ...
  tag: { fontSize: 11, color: "#0891B2", backgroundColor: "#ECFEFF", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2 },
