...
export default function MlaDashboard() {
  ...
  const [stats, setStats] = useState<DashStats>({
    ...
    resolvedGrievances: 0, upcomingEvents: 0,
  });
  ...
  const load = useCallback(async () => {
    try {
      const [dashRes, grievRes, eventsRes] = await Promise.all([
        api.get("/api/dashboard/mla").catch(() => ({ data: {} })),
        api.get("/api/grievances/stats/summary").catch(() => ({ data: {} })),
        api.get("/api/events/?per_page=5").catch(() => ({ data: [] })),
      ]);
      const d = dashRes.data || {};
      const g = grievRes.data || {};
      const byStatus: Record<string, number> = g.byStatus || {};
      const evList = eventsRes.data?.items ?? eventsRes.data ?? [];
      setStats({
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <View>
          <Text style={styles.greeting}>MLA Dashboard</Text>
      ...
      <View style={styles.kpiGrid}>
        <KPICard label="Total Complaints" value={stats.totalGrievances} color="#059669" icon="📋" />
        ...
        <KPICard label="Events" value={stats.upcomingEvents} color="#F59E0B" icon="📅" />
      </View>

      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Quick Actions</Text>
        <View style={styles.actionGrid}>
          <ActionCard icon="📋" title="Complaints" color="#ECFDF5" onPress={() => router.push("/mla/complaints" as any)} />
          <ActionCard icon="📅" title="Events" color="#FFF7ED" onPress={() => router.push("/mla/events" as any)} />
          <ActionCard icon="📊" title="Reports" color="#EFF6FF" onPress={() => router.push("/mla/complaints" as any)} />
          <ActionCard icon="👤" title="My Profile" color="#F5F3FF" onPress={() => router.push("/mla/profile" as any)} />
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: 20, paddingTop: 52, paddingBottom: 20, backgroundColor: "#059669",
  },
  greeting: { fontSize: 20, fontWeight: "700", color: "#fff" },
  subGreeting: { fontSize: 13, color: "#A7F3D0", marginTop: 2 },
  logoutText: { color: "#A7F3D0", fontSize: 13 },
