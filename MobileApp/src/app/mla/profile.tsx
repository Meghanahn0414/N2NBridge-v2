...
export default function MlaProfile() {
  ...
  async function loadProfile() {
    try {
      ...
      setProfile({ fullName: user?.name || "MLA", email: user?.email || "", constituency: "" });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.roleBadge}>
          <Text style={styles.roleText}>MLA / Representative</Text>
        </View>
      </View>
      ...
      <View style={styles.card}>
        ...
        {profile?.constituency && <InfoRow icon="🗺️" label="Constituency" value={profile.constituency} />}
        {profile?.createdAt && (
          <InfoRow icon="📅" label="Member Since"
      ...
      <View style={styles.card}>
        ...
        <MenuRow icon="📋" label="Constituency Complaints" onPress={() => router.push("/mla/complaints" as any)} />
        <MenuRow icon="📅" label="Events" onPress={() => router.push("/mla/events" as any)} />
      </View>
      ...
      <Text style={styles.version}>Jan Seva CRM · MLA App</Text>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: "#059669", paddingTop: 56, paddingBottom: 32, alignItems: "center" },
  avatarCircle: {
