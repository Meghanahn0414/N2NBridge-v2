...
class TokenManager:
    ...
    @staticmethod
    def create_token(user_id: str, role: str, db_name: str = "") -> str:
        """
        Create access token.
        db_name identifies which tenant database this session belongs to.
        """
        data = {
            "user_id": user_id,
            "role": role,
    ...
    def extract_token_from_header(authorization_header: str) -> Optional[str]:
        ...
        logger.debug(f"[JWT] Token extracted, length: {len(parts[1])}")
        return parts[1]
