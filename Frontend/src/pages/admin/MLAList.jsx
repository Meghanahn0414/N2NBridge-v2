...
const styles = {
  ...
  avatar: { width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #4f46e5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', flexShrink: 0 },
  name: { fontWeight: 600, color: '#0f172a', fontSize: 13 },
  roleBadge: { display: 'inline-block', padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, background: '#f0fdf4', color: '#16a34a' },
        ...
        <div style={styles.card}>
          ...
          {!loading && !error && (
            <table style={styles.table}>
              ...
              <tbody>
                {users.length === 0 ? (
                  <tr><td colSpan={8} style={styles.empty}>No representatives found</td></tr>
                ) : (
                  users.map((u, i) => {
                    ...
                    return (
                      <tr key={u._id || u.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbfc' }}>
                        ...
                        <td style={styles.td}>{u.constituencyId || u.constituency || '—'}</td>
                        <td style={styles.td}>{u.district || '—'}</td>
                        <td style={styles.td}><span style={styles.roleBadge}>{u.role || 'REPRESENTATIVE'}</span></td>
                        <td style={styles.td}>
